...
const buildEstimatedFlights = ({ from, to, date }) => {
  ...
  return carriers.map((carrier, index) => {
    ...
    const departureAt = toLocalIso(date, departureTimes[index]);
    const arrivalAt = toLocalIso(date, departureTimes[index], durationMinutes);
    ...
    return {
      ...
      itineraries: [{
        ...
        segments: [{
          departure: { iataCode: from, at: departureAt },
          arrival: { iataCode: to, at: arrivalAt },
...
module.exports = { buildEstimatedFlights };
